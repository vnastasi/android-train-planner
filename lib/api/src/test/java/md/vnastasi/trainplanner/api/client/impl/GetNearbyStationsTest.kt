...
internal class GetNearbyStationsTest : KoinTest {
    ...
    """)
    internal fun testQueryParameters(webServer: MockWebServer) {
        webServer.enqueueResponse {
            httpStatus = HttpURLConnection.HTTP_NOT_FOUND
        }

        assertThatBlocking { get<StationsApiClient>().getNearbyStations(LATITUDE, LONGITUDE, LIMIT).toCollection(mutableListOf()) }.isFailure()

        assertThat(webServer.takeRequest().requestUrl).all {
            isNotNull()
    ...
    internal fun testServerError(webServer: MockWebServer) {
        ...
        assertThatBlocking { get<StationsApiClient>().getNearbyStations(LATITUDE, LONGITUDE, LIMIT).toCollection(mutableListOf()) }
                .isFailure()
    ...
    internal fun testNonEmptyList(webServer: MockWebServer) {
        ...
        assertThatBlocking { get<StationsApiClient>().getNearbyStations(LATITUDE, LONGITUDE, LIMIT).toCollection(mutableListOf()) }
                ...
                .all {
                    hasSize(1)
                    prop("element") { it[0] }.contains(DistanceAwareStation(expectedStation, 123.45))
                }
    }
}

...
internal class GetNearbyStationsTest : KoinTest {
    ...
    internal fun testQueryParameters(webServer: MockWebServer) = runBlocking {
        ...
        get<StationsApiClient>().getNearbyStations(LATITUDE, LONGITUDE, LIMIT).test {
            awaitError()
        }
    ...
    internal fun testNonEmptyList(webServer: MockWebServer) = runBlocking {
        ...
        get<StationsApiClient>().getNearbyStations(LATITUDE, LONGITUDE, LIMIT).test {
            assertThat(awaitItem()).isNotNull().containsExactly(DistanceAwareStation(expectedStation, 123.45))
            awaitComplete()
        }
